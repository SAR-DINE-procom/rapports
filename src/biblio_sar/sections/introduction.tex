...
Bien que l’imagerie radar conventionnelle soit performante, elle présente une limite majeure : la résolution. En effet, dans le cas d’un SLAR/RAR\footnote{Real Aperture Radar ou radar à ouverture réelle}, la résolution en azimut peut être approximée par l’expression suivante :

\begin{equation}
\Delta x_{az} \approx \frac{R\lambda}{L}
\end{equation}
Où :
\begin{description}
\item[$\Delta x_{az}$] : résolution en azimut
\item[$R$] : distance entre la cible et le radar
\item[$\lambda$] : longueur d’onde
\item[$L$] : longueur physique de l’antenne
\end{description}
