...
Un système radar émet une onde électromagnétique qui se propage dans l'espace selon la directivité de l'antenne, comme le montrent les traits bleus de la figure \ref{fig:schema_explicatif_radar}. Lorsque cette onde rencontre une cible, des échos sont émis ; ce sont les traits jaunes de la même figure. Ces échos se propagent dans l'espace dans de multiples directions, amplitudes et phases. Ils peuvent également se heurter à d'autres surfaces, générant ainsi de nouveaux échos. Une partie des échos de l'onde incidente est captée par l'antenne du radar et analysée afin d'obtenir plusieurs informations. Il est possible d'extraire la distance relative de la cible à l'antenne, sa vitesse ou encore sa structure. 
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}
    % --- Éléments centraux ---
    \node (radar) at (0,0) {\scalebox{-1}[1]{\includegraphics[width=1.5cm]{src/biblio_sar/img/radar-de-vitesse.png}}};
    ...
    \begin{scope}[yshift=1.5cm]
        ...
        \node (truck_approaching) at (9,0) {\scalebox{-1}[1]{\includegraphics[width=1.5cm]{src/biblio_sar/img/truck-icon.png}}};
        \draw[-latex, thick] (truck_approaching) -- ++(-1.5,0);
    \end{scope}
    % --- Scénario 2 : Le véhicule s'éloigne ---
    \begin{scope}[yshift=-1.5cm]
        \node (truck_receding) at (8.25,0) {\includegraphics[width=1.5cm]{src/biblio_sar/img/truck-icon.png}};
...
\FloatBarrier
% Représentation impulsions simples avec écho
\begin{figure}[ht]
  \centering
  ...
  \begin{picture}(10,2)
    % fleche gauche
    {\color{blue}
        % Ligne entre l'écho et l'impulsion pour le tau_echo
        \put(0.775,0.125){\line(0,1){0.25}}
        \put(0.75,0.25){\line(1,0){3.25}}
        \put(3.975,0.125){\line(0,1){0.25}}
        \put(2.25,0.5){\makebox(0,0)[b]{$\tau_e$}}
        % Ligne entre les deux impulsions
        \put(0.25,1.5){\line(1,0){6}}
        \put(0.275,1.375){\line(0,1){0.25}}
        \put(6.225,1.375){\line(0,1){0.25}}
        \put(3,1.75){\makebox(0,0)[b]{$T_p$}}
        % Ligne periode pulse
        \put(6.25,1){\line(1,0){0.5}}
        \put(6.275,0.875){\line(0,1){0.25}}
        \put(6.725,0.875){\line(0,1){0.25}}
        \put(6.5,0.5){\makebox(0,0)[b]{$\tau_p$}}
    }
    \thicklines
    ...
    % Période de silence
    \put(0.75,0){\line(1,0){3.25}}
    % Echo d'une cible
    {\color{red}
        \put(4,0){\line(0,1){0.5}}
        \put(4,0.5){\line(1,0){0.5}}
        \put(4.5,0.5){\line(0,-1){0.5}}
    }
    % Période de silence
    \put(4.5,0){\line(1,0){1.75}}
...
L'utilisation du pic de corrélation pour détecter l'écho implique que si deux cibles sont proches comparés à la durée d'impulsions, elles seront confondues. Ainsi la résolution en distance est exprimée par l'équation suivante \cite{radartutorial} :

%La largeur des impulsions conditionne la résolution du radar. Plus elles sont larges, plus les cibles doivent être éloignées pour être détectées. Cependant, de larges impulsions permettent d'émettre plus de puissance et donc d'observer plus loin. A l'inverse, de fines impulsions permettront de détecter deux cibles très proches mais à faible distance \cite{radartutorial}. 
...
%Cette forme d'onde permet de mesurer la distance en calculant le temps que l'écho met à revenir après avoir rencontré une cible. En effet, puisque le canal de propagation de l'onde électromagnétique émise par le système radar est constant et que la vitesse d'une OEM dans l'air est connue et constante, il est possible de calculer la distance. Dans la figure \ref{fig:impulsions_radar_avec_echo}, l'écho d'une cible est représenté en rouge et le temps nécessaire à sa détection par $\tau_{echo}$. Elle est généralement utilisée par des radars de surveillance aérienne, météorologiques et les systèmes de navigation. 

Une autre problématique de cette forme d'onde est l'ambiguïté concernant la distance relative de la cible au radar. En effet, un écho résultant d'une impulsion peut être détecté après l'impulsion suivante. Ainsi, il n'est pas possible de déterminer la distance de la cible, la résolution maximale en distance d'un radar à impulsion s'exprime de la manière suivante \cite{radartutorial} : 
...
Où :
\begin{description}
    \item[$\delta_{max}$] : est la résolution maximale en distance [$m$]
    \item[$c_0$] : est la célérité d'une OEM dans le vide [$m.s^{-1}$]
    \item[$T_p$] : est le temps de répétition des impulsions [$m$]
    \item[$\tau_p$] : est la durée d'émission d'une impulsion [$s$]
\end{description}
...
On remarque que pour obtenir une bonne résolution en vitesse, il faut une durée d'impulsion $\tau_p$ très grande. Or, c'est l'exact opposé du critère permettant l'obtention d'une bonne résolution en distance. Il faut donc réaliser un compromis entre les deux critères ou bien utiliser d'autres formes d'ondes. 

\subsubsection{Impulsions modulées}

[A REDIGER] Mesure de la distance et de la vitesse mais il y a une ambiguïtés entre les deux résultats, utilisés dans des systèmes radars spécifiques (surveillance maritime et applications industrielles)
...
\begin{figure}[ht]
    ...
    {\color{blue}
        % Ligne entre l'écho et l'impulsion pour le tau_echo
        \put(0.25,0.125){\line(0,1){0.25}}
        \put(0.25,0.25){\line(1,0){0.5}}
        \put(0.75,0.125){\line(0,1){0.25}}
        \put(0.25,0.75){\makebox(0,0)[b]{$\tau_{echo}$}}
        % Ligne entre les deux impulsions
        \put(2,1.5){\line(1,0){2}}
        \put(2.025,1.375){\line(0,1){0.25}}
        \put(3.975,1.375){\line(0,1){0.25}}
        \put(3,1.75){\makebox(0,0)[b]{$T_{total}$}}
    }
    \thicklines
    % Impulsions chirp avec montée inclinée et descente verticale
    \put(0,0){\line(1,1){2}}    % montée linéaire
    \put(2,0){\line(0,1){2}}    % descente verticale
    ...
    % signal reçu
    {\color{red}
    \put(0.5,0){\line(1,1){2}}    % montée linéaire
    \put(2.5,0){\line(0,1){2}}    % descente verticale

    \put(2.5,0){\line(1,1){2}}
    \put(4.5,0){\line(0,1){2}}

    \put(4.5,0){\line(1,1){2}}
    \put(6.5,0){\line(0,1){2}}

    \put(6.5,0){\line(1,1){2}}
    \put(8.5,0){\line(0,1){2}}

    \put(8.5,0){\line(1,1){1.5}}
    %\put(10,0){\line(0,1){2}}
    }
...
\subsubsection{Ondes continues}

[A REDIGER]Mesure simultanées vitesse et distance, utilisée dans les radars de controle de la circulation, systèmes de détection de collision et les capteurs de distance
[TODO] Ajuster la fmcw pour ajouter un tau temporel et un changement d'amplitude
\begin{figure}[ht]
    \centering
    \setlength{\unitlength}{1cm}
    \begin{picture}(10,2)
    ...
    {\color{blue}
        ...
        % Ligne entre les deux impulsions
        \put(2,2){\line(1,0){4}}
        \put(2,1.875){\line(0,1){0.25}}
        \put(6,1.875){\line(0,1){0.25}}
        \put(4,1.5){\makebox(0,0)[b]{$T_{total}$}}
    }
    
    \thicklines
    % Impulsions chirp avec montée inclinée et descente verticale
    \put(0,0){\line(1,1){2}}    % montée linéaire
    \put(2,2){\line(1,-1){2}}    % descente linéaire
    \put(4,0){\line(1,1){2}}    
    \put(6,2){\line(1,-1){2}}
    \put(8,0){\line(1,1){2}}
    ...
    {\color{red}
    \put(0.5,0){\line(1,1){2}}    % montée linéaire
    \put(2.5,2){\line(1,-1){2}}    % descente linéaire
    \put(4.5,0){\line(1,1){2}}    
    \put(6.5,2){\line(1,-1){2}}
    \put(8.5,0){\line(1,1){1.5}}
    }
    
    \end{picture}
    \caption{Représentation de la forme d'onde FMCW et de son écho}
