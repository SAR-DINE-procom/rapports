...
La puissance reçue $P_r$ s’exprime selon l’« équation radar » classique \cite{richards2010} :
...
\begin{equation}
    \delta_d \ge \frac{c_0\,\tau_p}{2} = \frac{c_0}{2\,B}
    \label{eq:resolution_distance_pulse}
\end{equation}
où $\tau_p$ est la durée de l’impulsion et $B$ sa bande de fréquence. Un compromis est nécessaire entre résolution en distance (favorisée par des impulsions courtes) et résolution en vitesse (favorisée par des impulsions longues), cette dernière s’exprimant :

\begin{equation}
    \delta\nu_r = \frac{\lambda_0}{2\,\tau_p} = \frac{\lambda_0\,B}{2}
...
La modulation linéaire (\emph{chirp} Sawtooth) fait varier la fréquence sur une bande $B_{ch}$, améliorant le pic de corrélation pour la résolution en distance :
...
Les résolutions s’expriment alors \cite{richards2010} :
...
Jusqu’à présent, le bruit a été négligé. Or, il est indispensable de le modéliser pour évaluer la performance réelle. Le \emph{signal} désigne les échos utiles, tandis que le \emph{bruit} englobe toutes les perturbations (interférences, hydrométéores, bruit thermique des composants). Le rapport signal-sur-bruit (SNR) se définit \cite{skolnik2001} :
...
Les figures \ref{fig:illus_snr_facteur_de_bruit} et \ref{fig:snr_degradation} illustrent l’évolution du SNR à travers une chaîne RF et l’importance du premier étage, qui conditionne la performance globale.
...
Le gain total d’une chaîne de $n$ éléments s’exprime par \cite{Friis1944Noise} :
\begin{equation}
    G_T = \prod_{i=1}^n G_i
    \label{eq:gain_total}
\end{equation}
...
La sélection soignée du premier composant est cruciale pour limiter la dégradation globale du SNR. 

Avec ces notions établies, la section \ref{sec:principes_sar} détaillera les principes spécifiques à l’imagerie radar, en particulier le SAR.
