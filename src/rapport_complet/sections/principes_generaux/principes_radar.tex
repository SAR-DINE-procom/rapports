...
D'un point de vue haut niveau, l'impact du bruit sur un système se caractérise par le rapport signal sur bruit (RSB \footnote{Rapport Signal sur Bruit}/SNR \footnote{Signal to Noise Ratio}). La dégradation du RSB au travers d'un dispositif/système se caractérise par le facteur de bruit.
...
\noindent Le facteur de bruit s'exprime de la manière suivante : 
...
Le facteur de bruit représente donc le rapport entre le RSB en entrée et le RSB en sortie. La deuxième partie de l'équation permet de définir $F$ par rapport à une référence de bruit thermique en entrée à $T_0 = 290 \degree K$, tel que $N_e = k.T_0.B$ avec $k$ la constante de Boltzmann et $B$ la bande de fréquence observée. Par ailleurs, on peut alors exprimer la température équivalente de bruit tel que :
...
De plus, le facteur de bruit d'un composant passif correspond à l'atténuation du composant tandis que pour un composant actif, il s'agit d'une donnée constructeur.

Une autre manière permettant d'aborder le rapport signal sur bruit et du facteur de bruit consiste à l'observer graphiquement. Considérons la figure \ref{fig:illus_snr_facteur_de_bruit}, cette dernière représente un système simple composé d'une antenne, d'un support de propagation et d'un amplificateur. La figure \ref{fig:snr_degradation} représente l'évolution du signal et du bruit à mesure que l'on se déplace au travers du système.
...
On remarque alors qu'à mesure que le signal progresse à travers le système, plus il est atténué, et plus le bruit augmente en fonction de l'atténuation. Le dernier composant amplifie également le bruit ; il n'est plus possible de distinguer une partie du signal. Le RSB s'est donc dégradé à mesure que le signal a progressé dans ce système. 

Un point important facilitant l'étude des facteurs de bruit d'un système est la possibilité de les mettre en cascade. Pour cela, on commence par représenter élément de la chaîne par son gain et son facteur de bruit. Grâce à la formule de FRIIS pour le bruit, on est capable d'exprimer à la fois le gain total et le facteur de bruit total d'un système. 

Le gain total s'exprime ainsi \cite{Friis1944Noise} :  
\begin{equation}
    G_T = G_1.G_2.\dots.G_n
    \label{eq:gain_total}
\end{equation}
Où :
\begin{description}
    \item[$G_T$] : Le gain total
    \item[$G_x$] : Le gain de l'élément $x$
\end{description}
\vspace{1em}

Et le facteur de bruit total \cite{Friis1944Noise} :
\begin{equation}
    F_T = F_1 + \frac{F_2 - 1}{G_1} + \frac{F_3 - 1}{G_1.G_2} + \dots + \frac{F_n - 1}{\Pi_{k=1}^{n-1} G_k}
    \label{eq:formule_friis_bruit}
\end{equation}
Où :
\begin{description}
    \item[$F_T$] : Le facteur de bruit total
    \item[$F_x$] : Le facteur de bruit de l'élément $x$
    \item[$G_x$] : Le gain de l'élément $x$
\end{description}
\vspace{1em}

On remarque que la première source de bruit du système sera la plus impactante sur le RSB, il faut donc choisir avec attention le premier élément d'une chaîne RF.
